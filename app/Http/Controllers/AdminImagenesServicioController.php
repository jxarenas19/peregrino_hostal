...
class AdminImagenesServicioController extends CBController {

    use GenericController;

    public function cbInit()
    {
        $this->setTable("hp_images_service");
        $this->setPermalink("imagen_servicio");
        $this->setPageTitle("Imagen Servicio");

        $this->addText("Nombre","name-visual")->required(false)->showDetail(false)
            ->showAdd(false)->showEdit(false)->strLimit(150)->maxLength(255);
